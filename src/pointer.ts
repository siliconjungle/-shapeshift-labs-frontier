...
let lastPointerPathKey: string | null = null;
let lastPointerPathValue: JsonPath | null = null;
...
export function stringifyPointer(path: JsonPath): string {
  if (!Array.isArray(path)) throw new TypeError('JSON Pointer path must be an array');
  let pointer = '';
...
export function getPath(value: JsonValue, path: JsonPath): JsonValue | undefined {
...
export function findPointerParent(value: JsonValue, pointer: string): [JsonObject | JsonArray, PathSegment] | null {
...
export function getCachedPointerPath(pointer: string): JsonPath {
  if (pointer === lastPointerPathKey) return lastPointerPathValue as JsonPath;
  ...
  if (pointerPathCache.size >= MAX_POINTER_CACHE_SIZE) pointerPathCache.clear();
...
function parseCachedPointerPath(pointer: string): JsonPath {
  if (pointer === '') return [];
  assertPointerString(pointer);
...
function readCachedPointerSegment(pointer: string, start: number, end: number): string | number {
  const segment = decodePointerSegment(pointer, start, end);
  return toCanonicalNumericSegment(segment);
}

export function decodePointerSegment(pointer: string, start: number, end: number): string {
  let tilde = -1;
...
export function readArrayIndex(key: string | number, length: number, allowAppend: boolean): number {
  if (typeof key === 'number') {
    if (!Number.isSafeInteger(key) || key < 0) throw new RangeError('invalid JSON Pointer array index: ' + key);
  ...
  if (text === '') throw new RangeError('invalid JSON Pointer array index');
  ...
  for (let i = 0, keyLength = text.length; i < keyLength; i++) {
    ...
    if (digit < 0 || digit > 9) throw new RangeError('invalid JSON Pointer array index: ' + text);
    index = index * 10 + digit;
    if (!Number.isSafeInteger(index)) throw new RangeError('JSON Pointer array index is not a safe integer: ' + text);
  }
...
function toCanonicalNumericSegment(segment: string): string | number {
  const length = segment.length;
  if (length === 0) return segment;

  let code = segment.charCodeAt(0);
  if (code === 48) return length === 1 ? 0 : segment;
