...
function applySameParentSetMutableFastPath(root, patch) {
  const first = patch[0];
  if (first[0] !== OP_SET) return NO_MUTABLE_FAST_PATH;

  const firstPath = first[1];
  const pathLength = firstPath.length;
  if (pathLength === 0 || pathLength > 128) return NO_MUTABLE_FAST_PATH;
  ...
  if (parent === null || typeof parent !== 'object') return NO_MUTABLE_FAST_PATH;
...
export function applyPatchImmutable(value: JsonValue, patch: Patch): JsonValue {
  ...
  if (length === 1 && patch[0][0] === OP_SCALAR_ARRAY_REPLACE) {
    const op = patch[0];
    if (op[1].length === 0) return op[2].slice();
    if (value !== null && typeof value === 'object') {
      const replaced = applySingleScalarArrayReplacePathCopy(value, op[1], op[2]);
      if (replaced !== NO_IMMUTABLE_FAST_PATH) return replaced;
    }
  }
  ...
  if (length === 1 && Array.isArray(value) && patch[0][1].length === 0) {
    ...
    if (fastRootArray !== NO_IMMUTABLE_FAST_PATH) return fastRootArray;
  }
  ...
  if (length > 1 && length <= 16 && Array.isArray(value)) {
    ...
    if (removeMoveInsertSet !== NO_IMMUTABLE_FAST_PATH) return removeMoveInsertSet;

    const removeInsertSet = applyRootArrayRemoveTwoFieldInsertSetPatchImmutable(value, patch);
    if (removeInsertSet !== NO_IMMUTABLE_FAST_PATH) return removeInsertSet;

    const twoSplicesSet = applyRootArrayTwoSplicesSetPatchImmutable(value, patch);
    if (twoSplicesSet !== NO_IMMUTABLE_FAST_PATH) return twoSplicesSet;

    const smallInsert = applyRootArraySmallInsertPatchImmutable(value, patch);
    if (smallInsert !== NO_IMMUTABLE_FAST_PATH) return smallInsert;

    const fast = applyRootArrayPatchImmutableFast(value, patch);
    if (fast !== NO_IMMUTABLE_FAST_PATH) return fast;
  }
  ...
  if (length > 1 && value !== null && typeof value === 'object' && !Array.isArray(value)) {
    ...
    if (rootObjectFast !== NO_IMMUTABLE_FAST_PATH) return rootObjectFast;
  }

  if (length >= 8 && value !== null && typeof value === 'object') {
    const sameParentSetRemove = applySameParentSetRemovePatchImmutable(value, patch);
    if (sameParentSetRemove !== NO_IMMUTABLE_FAST_PATH) return sameParentSetRemove;

    const sameParentAssign = applySameParentAssignPatchImmutable(value, patch);
    if (sameParentAssign !== NO_IMMUTABLE_FAST_PATH) return sameParentAssign;
  }

  if (length > 1 && value !== null && typeof value === 'object') {
    const sameArraySplicePath = applySameArraySplicePathImmutable(value, patch);
    if (sameArraySplicePath !== NO_IMMUTABLE_FAST_PATH) return sameArraySplicePath;
  }
...
function applyRootStringPatchFast(value, patch) {
  if (patch.length === 2) {
    ...
    if (rotated !== NO_MUTABLE_FAST_PATH) return rotated;
  }

  if (patch.length > 1) {
    const equalWidthReplacements = applyRootStringEqualWidthReplacementRun(value, patch);
    if (equalWidthReplacements !== NO_MUTABLE_FAST_PATH) return equalWidthReplacements;
  }
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    const op = patch[i];
    if (op[1].length !== 0) return NO_MUTABLE_FAST_PATH;
...
function applyRootStringEqualWidthReplacementRun(value, patch) {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (start < cursor) return NO_MUTABLE_FAST_PATH;
...
function applySingleScalarArrayReplacePathCopy(originalRoot, path, values) {
  if (path.length === 0) return values.slice();
  if (path.length > 128) return NO_IMMUTABLE_FAST_PATH;
  const parentPath = path.slice(0, path.length - 1);
  const cloned = cloneContainerPathForSingle(originalRoot, parentPath);
  if (cloned === null) return NO_IMMUTABLE_FAST_PATH;
...
function applyRootObjectRemoveAssignPatchImmutable(originalRoot, patch) {
  ...
  if (last[0] !== OP_ASSIGN || last[1].length !== 0) return NO_IMMUTABLE_FAST_PATH;

  for (let i = 0; i < length - 1; i++) {
    const op = patch[i];
    const path = op[1];
    if (op[0] !== OP_REMOVE || path.length !== 1) return NO_IMMUTABLE_FAST_PATH;
  }
...
function shouldUsePathCopy(root, patch) {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (path.length === 0 && patch[i][0] === OP_SET) return false;
...
function applyPatchPathCopy(originalRoot, patch) {
  ...
  function enableStructuralTracking() {
    if (clonedContainers !== null) return;
    clonedContainers = new WeakSet();
    structuralArrays = new WeakSet();
    if (root !== null && typeof root === 'object') clonedContainers.add(root);
  }
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (code === OP_SET) {
      ...
      if (op[3] !== op[4].length) enableStructuralTracking();
      if (path.length === 0 && Array.isArray(root)) {
        if (root === originalRoot) {
          root = applyRootArraySpliceImmutable(root, op[2], op[3], op[4]);
          if (clonedContainers !== null) clonedContainers.add(root);
        } else {
          applyArraySplice(root, op[2], op[3], op[4], true);
        }
        if (op[3] !== op[4].length) structuralArrays.add(root);
      } else {
        clonePath(root, originalRoot, path, path.length, cloned, clonedContainers, structuralArrays);
        root = cloned[0];
        applyArraySplice(cloned[1], op[2], op[3], op[4], true);
        if (op[3] !== op[4].length) structuralArrays.add(cloned[1]);
      }
    } else if (code === OP_ARRAY_TWO_FIELD_INSERT) {
...
function clonePath(root, originalRoot, path, depth, out, clonedContainers, structuralArrays) {
  ...
  for (let i = 0; i < depth; i++) {
    ...
    if (
      ...
      if (clonedContainers !== null) clonedContainers.add(clonedChild);
...
function applySameArraySplicePathImmutable(originalRoot, patch) {
  const first = patch[0];
  if (first[0] !== OP_ARRAY_SPLICE) return NO_IMMUTABLE_FAST_PATH;
  const path = first[1];
  if (path.length > 128) return NO_IMMUTABLE_FAST_PATH;
  ...
  if (cloned === null || !Array.isArray(cloned[1])) return NO_IMMUTABLE_FAST_PATH;
...
function applySingleCompactPathCopy(originalRoot, op) {
  ...
  if (!Array.isArray(target)) return NO_IMMUTABLE_FAST_PATH;
  if (code === OP_ARRAY_ASSIGN) {
...
function applySameParentSetRemovePatchImmutable(originalRoot, patch) {
  ...
  if (firstCode !== OP_SET && firstCode !== OP_REMOVE) return NO_IMMUTABLE_FAST_PATH;
  const firstPath = first[1];
  if (firstPath.length === 0 || firstPath.length > 128) return NO_IMMUTABLE_FAST_PATH;
  const depth = firstPath.length - 1;

  for (let i = 1, length = patch.length; i < length; i++) {
    const op = patch[i];
    const code = op[0];
    if (code !== OP_SET && code !== OP_REMOVE) return NO_IMMUTABLE_FAST_PATH;
    const path = op[1];
...
function applySameParentAssignPatchImmutable(originalRoot, patch) {
  const first = patch[0];
  if (first[0] !== OP_ASSIGN) return NO_IMMUTABLE_FAST_PATH;
  const firstPath = first[1];
  if (firstPath.length === 0 || firstPath.length > 128) return NO_IMMUTABLE_FAST_PATH;
  const depth = firstPath.length - 1;
  ...
  for (let i = 1, length = patch.length; i < length; i++) {
    const op = patch[i];
    if (op[0] !== OP_ASSIGN) return NO_IMMUTABLE_FAST_PATH;
    const path = op[1];
    if (path.length !== firstPath.length || !samePathPrefix(firstPath, path, depth)) {
      return NO_IMMUTABLE_FAST_PATH;
    }
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (child === null || typeof child !== 'object') return NO_IMMUTABLE_FAST_PATH;
...
function applyRootArrayPatchImmutableFast(originalRoot, patch) {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (path.length === 0) {
      ...
      const index = path[0];
      if (typeof index !== 'number') return NO_IMMUTABLE_FAST_PATH;

      if (code === OP_SET) {
        root[index] = clonePatchValue(op[2]);
      } else if (code === OP_ASSIGN) {
        const row = cloneRootArrayObject(root, index);
        if (row === null) return NO_IMMUTABLE_FAST_PATH;
...
function canApplyRootArrayPatchImmutableFast(patch) {
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (path.length === 0) {
      ...
      if (typeof path[0] !== 'number' || (code !== OP_SET && code !== OP_ASSIGN && code !== OP_REMOVE)) return false;
    } else if (path.length === 2) {
      if (typeof path[0] !== 'number' || (code !== OP_SET && code !== OP_REMOVE)) return false;
    } else {
      return false;
    }
  }
...
function applyRootArrayTwoSplicesSetPatchImmutable(originalRoot, patch) {
  if (patch.length !== 3) return NO_IMMUTABLE_FAST_PATH;
  const first = patch[0];
  const second = patch[1];
  const third = patch[2];
  if (
    first[0] !== OP_ARRAY_SPLICE ||
  ...
  if (direct !== NO_IMMUTABLE_FAST_PATH) return direct;
...
function applyRootArrayTwoSplicesSetDirect(originalRoot, first, second, third) {
  ...
  if (inserted === null) return NO_IMMUTABLE_FAST_PATH;
...
function applyRootArrayRemoveMoveInsertSetPatchImmutable(originalRoot, patch) {
  if (patch.length !== 4) return NO_IMMUTABLE_FAST_PATH;
  const remove = patch[0];
  const move = patch[1];
  const insert = patch[2];
  const set = patch[3];
  if (
    remove[0] !== OP_ARRAY_SPLICE ||
...
function applyRootArraySmallInsertPatchImmutable(originalRoot, patch) {
  ...
  if (insertOp === null) return NO_IMMUTABLE_FAST_PATH;
  ...
  for (let j = 0, patchLength = patch.length; j < patchLength; j++) {
    const op = patch[j];
    if (op !== insertOp) root[op[1][0]] = op[2];
  }
  return root;
}
...
function arePatchScalars(values) {
  for (let i = 0, length = values.length; i < length; i++) {
    if (!isPatchScalar(values[i])) return false;
  }
  return true;
}
...
function cloneContainerPathForSingle(originalRoot, path) {
  ...
  for (let i = 0, length = path.length; i < length; i++) {
    const key = path[i];
    if (originalNode === null || typeof originalNode !== 'object') return null;
    const originalChild = originalNode[key];
    if (originalChild === null || typeof originalChild !== 'object') return null;
    const clonedChild = shallowCloneContainer(originalChild);
    assignChild(clonedNode, key, clonedChild);
    originalNode = originalChild;
    clonedNode = clonedChild;
...
function clonePatchObject(value) {
  ...
  if (queryRow !== null) return queryRow;

  const scalarObject = cloneSmallScalarPatchObject(value);
  if (scalarObject !== null) return scalarObject;
...
function cloneSmallScalarPatchObject(value) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!isPatchScalar(item)) return null;
    if (key === '__proto__') {
      Object.defineProperty(out, key, {
        value: item,
        enumerable: true,
...
function cloneIdValueScalarObject(value) {
  if (hasOwn.call(value, '__proto__')) return null;
  if (!hasOwn.call(value, 'id') || !hasOwn.call(value, 'value')) return null;
  const keys = Object.keys(value);
  return cloneIdValueScalarObjectWithKeys(value, keys);
}

function cloneIdValueScalarObjectWithKeys(value, keys) {
  if (keys.length !== 2 || !hasOwn.call(value, 'id') || !hasOwn.call(value, 'value')) return null;
  const id = value.id;
  const nextValue = value.value;
  if (!isPatchScalar(id) || !isPatchScalar(nextValue)) return null;
...
function cloneRootArrayRowWithFastScalarSet(row, key, value) {
  ...
  if (key === 'value') {
    ...
    if (directSet !== null) return directSet;
...
function cloneExactIdValueRowWithValue(row, value) {
  ...
  if (count !== 2 || !hasId || !hasValue) return null;
...
function applyRootArrayTwoFieldInsertImmutable(array, start, key0, key1, values0, values1) {
  if (values0.length === 1) {
    ...
    if (typeof array.toSpliced === 'function') return array.toSpliced(start, 0, row);
...
function makeTwoFieldRows(key0, key1, values0, values1) {
  ...
  if (key0 === 'position' && key1 === 'char') {
    for (let i = 0; i < length; i++) out[i] = { position: values0[i], char: values1[i] };
    return out;
  }
  if (key0 === 'id' && key1 === 'value') {
    for (let i = 0; i < length; i++) out[i] = { id: values0[i], value: values1[i] };
    return out;
  }
  for (let i = 0; i < length; i++) {
    const row = {};
...
function makeTwoFieldRow(key0, key1, value0, value1) {
  if (key0 === 'position' && key1 === 'char') return { position: value0, char: value1 };
  if (key0 === 'id' && key1 === 'value') return { id: value0, value: value1 };
  const row = {};
...
function applyRootArraySpliceImmutable(array, start, deleteCount, values) {
  ...
  if (deleteCount === 0) {
    ...
    if (insertCount >= 16) {
      ...
      if (smallObjectInsert !== null) return smallObjectInsert;
    }
    ...
    if (insertCount <= 4) {
      ...
      let i = 0;
      for (; i < start; i++) out[i] = array[i];
      for (let j = 0; j < insertCount; j++) out[i + j] = clonePatchValue(values[j]);
      for (; i < length; i++) out[i + insertCount] = array[i];
      return out;
    }
  ...
  if (insertCount === deleteCount) {
    if (insertCount >= LONG_EQUAL_ARRAY_SPLICE_VALUE_COUNT) {
      ...
      for (let i = 0; i < insertCount; i++) out[start + i] = values[i];
    } else {
      for (let i = 0; i < insertCount; i++) {
...
function applyRootSmallObjectRowsInsert(array, start, values, insertCount) {
  ...
  if (twoFieldRows !== null) return twoFieldRows;
  ...
  const out = new Array(length + insertCount);
  let offset = 0;
  for (; offset < start; offset++) out[offset] = array[offset];
  ...
  for (let i = start; i < length; i++) out[offset++] = array[i];
  return out;
}
...
function applyRootTwoFieldScalarRowsInsert(array, start, values, insertCount, keys) {
  ...
  if (key0 === 'position' && key1 === 'char') {
    ...
    for (let i = 0; i < insertCount; i++) {
      ...
      if (!isPatchScalar(position) || !isPatchScalar(char)) return null;
...
function applyRootArrayInsertValues(array, start, inserted, insertCount) {
  ...
  let offset = 0;
  for (; offset < start; offset++) out[offset] = array[offset];
  for (let i = 0; i < insertCount; i++) out[offset++] = inserted[i];
  for (let i = start; i < length; i++) out[offset++] = array[i];
  return out;
}
...
function clonePatchValues(values) {
  ...
  if (length >= 16) {
    ...
    if (smallObjectRows !== null) return smallObjectRows;
  }
...
function readSmallScalarObjectKeys(value) {
  ...
  for (let i = 0, length = keys.length; i < length; i++) {
    if (!isPatchScalar(value[keys[i]])) return null;
  }
  return keys;
}
...
function cloneSmallScalarObjectWithKeys(value, keys) {
  ...
  if (!hasExactPatchKeys(value, keys)) return null;
  const out = {};
  for (let i = 0, length = keys.length; i < length; i++) {
    const key = keys[i];
    const item = value[key];
...
function assignArrayObjectFieldValuesPlanned(array, rowIndexes, values, cloneValues, plan) {
  ...
  if (nestedKeys.length === 0) {
    ...
    if (directKeys.length === 2) {
      assignArrayObjectFieldValuesDirect2(
        array,
        rowIndexes,
        values,
        directKeys[0],
        directValueIndexes[0],
        directKeys[1],
        directValueIndexes[1],
        fieldCount,
        cloneValues
      );
      return;
...
function assignArrayObjectFieldValuesImmutablePlanned(array, rowIndexes, values, plan) {
  ...
  if (nestedKeys.length === 0) {
    ...
    if (directKeys.length === 2) {
      assignArrayObjectFieldValuesImmutableDirect2(
        array,
        rowIndexes,
        values,
        directKeys[0],
...
function readObjectFieldApplyPlan(fields) {
  ...
  if (cached !== undefined) return cached === NO_OBJECT_FIELD_APPLY_PLAN ? null : cached;
...
function assignRelativeField(row, field, value) {
  let parent = row;
  for (let i = 0, last = field.length - 1; i < last; i++) parent = parent[field[i]];
...
function applyStringInsertionRun(value, patch, start, end) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (sourceIndex < cursor || sourceIndex > value.length) return null;
    if (sourceIndex > cursor) parts[parts.length] = value.slice(cursor, sourceIndex);
    parts[parts.length] = op[4];
    cursor = sourceIndex;
    insertedLength += op[4].length;
  }

  if (cursor < value.length) parts[parts.length] = value.slice(cursor);
  return parts.join('');
}
...
function applyStringSinglePointInsertionRun(value, patch, start, end) {
  ...
  if (sourceIndex < 0 || sourceIndex > value.length) return null;

  let insertedLength = 0;
  let inserted = '';
  for (let i = start; i < end; i++) {
    const op = patch[i];
    if (op[2] - insertedLength !== sourceIndex) return null;
    inserted += op[4];
    insertedLength += op[4].length;
  }
