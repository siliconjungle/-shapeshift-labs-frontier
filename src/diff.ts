...
export function diff(source: JsonValue, target: JsonValue, options?: DiffOptions): Patch {
  if (options && options.strategy === 'replace') {
    ...
    if (
      source === target &&
      (typeof source !== 'number' || source !== 0 || 1 / source === 1 / (target as number))
    ) {
      return [];
    }
...
export function diffStable(source: JsonValue, target: JsonValue, options?: DiffOptions): Patch {
...
export function diffInto(source: JsonValue, target: JsonValue, patch: Patch, options?: DiffOptions): Patch {
...
function readMaxPatchOperations(options) {
  if (!options || options.maxPatchOperations === undefined || options.maxPatchOperations === null) return -1;
...
function diffDirtyRows(source, target, dirtyRows, patch, keyCompare, getVersion, arrayKey) {
  for (let i = 0, length = dirtyRows.length; i < length; i++) {
    ...
    diffDirtyRowsFallback(
      source,
      target,
      rows === frontier.rows ? frontier : { ...frontier, rows },
      patch,
      keyCompare,
      getVersion,
      arrayKey
...
function normalizeDirtyRowIndexes(rows) {
  ...
  if (ordered || length > DIRTY_ROW_NORMALIZE_MAX) return rows;

  const sorted = new Array(length);
  for (let i = 0; i < length; i++) sorted[i] = rows[i];
...
function tryDiffDirtyPathsAsCommonObjectSets(source, target, dirtyPaths, patch) {
  ...
  for (let i = 1, count = dirtyPaths.length; i < count; i++) {
    ...
    for (let segmentIndex = 0; segmentIndex < prefixLength; segmentIndex++) {
      if (path[segmentIndex] !== first[segmentIndex]) return false;
    }
  }
  ...
  for (let i = 0, count = dirtyPaths.length; i < count; i++) {
    ...
    if (sourceHasKey) {
      ...
      if (sourceValue === targetValue) {
        if (sourceValue !== 0 || 1 / sourceValue === 1 / targetValue) continue;
      }
  ...
  if (changed !== 0) patch[patch.length] = [OP_ASSIGN, parentPath, assigned];
  return true;
}
...
function tryDiffDirtyRowObjectFieldAssign(sourceRows, targetRows, rows, fields, path, patch) {
  ...
  if (rowCount < DIRTY_ARRAY_ROW_GROUP_MIN || fieldCount < 1 || fieldCount > 16) return false;
  if (!areCompactDirtyRowFieldPaths(fields)) return false;
  if (fieldCount === 1) {
  ...
  for (let rowOffset = 0; rowOffset < rowCount; rowOffset++) {
    ...
    for (let fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++) {
      ...
      if (
        sourceValue === MISSING_PATH_VALUE ||
      ...
      if (!sameJsonScalarOrRef(sourceValue, targetValue)) changedCount++;
    }

    if (changedCount !== 0) {
      if (changedCount * 2 < fieldCount) return false;
  ...
  if (indexes.length < DIRTY_ARRAY_ROW_GROUP_MIN) return false;
...
function areCompactDirtyRowFieldPaths(fields) {
  for (let i = 0, length = fields.length; i < length; i++) {
    ...
    if (!Array.isArray(field) || field.length === 0 || field.length > 2) return false;
    const key = field[0];
    if (typeof key !== 'string') return false;
    if (field.length === 2) {
      const childKey = field[1];
      if (typeof childKey !== 'string' && typeof childKey !== 'number') return false;
    }
  }
  return true;
}
...
function readCompactRowField(row, field) {
  ...
  const key = field[0];
  if (!hasOwn.call(row, key)) return MISSING_PATH_VALUE;
  const parent = row[key];
  if (parent === null || typeof parent !== 'object') return MISSING_PATH_VALUE;
  const childKey = field[1];
...
function tryDiffDirtyPathsAsRepeatedRowFields(source, target, dirtyPaths, patch) {
  if (!Array.isArray(dirtyPaths) || dirtyPaths.length < DIRTY_ARRAY_ROW_GROUP_MIN) return false;
  ...
  while (index < dirtyPaths.length) {
    ...
    if (index !== 0) validateDirtyPath(rowStart);
    if (!samePathPrefixLength(rowStart, first, rowDepth)) return false;

    const rowIndex = rowStart[rowDepth];
    if (!Number.isSafeInteger(rowIndex) || rowIndex < 0 || rowIndex <= previousRow) return false;

    let rowFieldCount = 0;
    while (index < dirtyPaths.length) {
      const path = dirtyPaths[index];
      if (path !== rowStart) validateDirtyPath(path);
      if (
      ...
      if (relativeLength < 1 || relativeLength > 2) return false;

      if (fieldCount === 0) {
        if (rowFieldCount >= 16) return false;
        ...
        if (rowFieldCount >= fieldCount) return false;
        const field = fields[rowFieldCount];
        if (
          field.length !== relativeLength ||
  ...
  if (rows.length < DIRTY_ARRAY_ROW_GROUP_MIN) return false;
...
function tryDiffDirtyPathsAsSingleRepeatedRowFieldPrefix(source, target, dirtyPaths, patch): number {
  if (!Array.isArray(dirtyPaths) || dirtyPaths.length < DIRTY_ARRAY_ROW_GROUP_MIN + 1) return 0;
  ...
  if (relativeLength < 1 || relativeLength > 2) return 0;
  ...
  if (nested && typeof childKey !== 'string' && typeof childKey !== 'number') return 0;
  ...
  for (let count = dirtyPaths.length; index < count; index++) {
    const path = dirtyPaths[index];
    if (index !== 0) validateDirtyPath(path);
    if (
      path.length !== expectedLength ||
      !samePathPrefixLength(path, first, rowDepth) ||
      path[rowDepth + 1] !== key ||
    ...
    let targetValue;
    if (nested) {
      if (!hasOwn.call(sourceRow, key) || !hasOwn.call(targetRow, key)) return 0;
      const sourceParent = sourceRow[key];
      const targetParent = targetRow[key];
      if (
        sourceParent === null ||
  ...
  if (index < DIRTY_ARRAY_ROW_GROUP_MIN) return 0;
  if (indexes.length !== 0) {
...
function tryDiffDirtyPathsAsFieldMajorRowFields(source, target, dirtyPaths, patch) {
  if (!Array.isArray(dirtyPaths) || dirtyPaths.length < DIRTY_ARRAY_ROW_GROUP_MIN * 2) return false;
  ...
  if (firstRelativeLength < 1 || firstRelativeLength > 2) return false;

  const firstExpectedLength = first.length;
  const firstField = copyDirtyRelativeField(first, rowDepth, firstRelativeLength);
  if (!isCompactDirtyRowFieldPath(firstField)) return false;
  ...
  while (index < dirtyPaths.length) {
    ...
    if (index !== 0) validateDirtyPath(path);
    if (
      path.length !== firstExpectedLength ||
      !samePathPrefixLength(path, first, rowDepth) ||
  ...
  const rowCount = rows.length;
  if (rowCount < DIRTY_ARRAY_ROW_GROUP_MIN || index === dirtyPaths.length) return false;
  ...
  while (index < dirtyPaths.length) {
    ...
    if (
      !isCompactDirtyRowFieldPath(field) ||
    ...
    for (let rowOffset = 0; rowOffset < rowCount; rowOffset++) {
      ...
      if (rowOffset !== 0) validateDirtyPath(path);
      if (
        path.length !== fieldStart.length ||
        !samePathPrefixLength(path, first, rowDepth) ||
...
function sameDirtyRelativeField(path, rowDepth, field) {
  if (path[rowDepth + 1] !== field[0]) return false;
...
function tryDiffDirtyPathsAsSingleRepeatedRowField(source, target, dirtyPaths, first, rowDepth, patch) {
  ...
  const nested = relativeLength === 2;
  const childKey = nested ? first[rowDepth + 2] : undefined;
  if (nested && typeof childKey !== 'string' && typeof childKey !== 'number') return false;
  ...
  for (let index = 0, count = dirtyPaths.length; index < count; index++) {
    ...
    let targetValue;
    if (nested) {
      if (!hasOwn.call(sourceRow, key) || !hasOwn.call(targetRow, key)) return false;
      const sourceParent = sourceRow[key];
      const targetParent = targetRow[key];
      if (
        sourceParent === null ||
...
function findDirtyPathRowDepth(path) {
  for (let i = path.length - 2; i >= 0; i--) {
    if (typeof path[i] === 'number' && typeof path[i + 1] === 'string') return i;
  }
  return -1;
}
...
function isCompactDirtyRowFieldPath(field) {
  if (field.length === 0 || field.length > 2 || typeof field[0] !== 'string') return false;
...
function tryDiffDirtyRowObjectFieldsAssign(sourceRows, targetRows, rows, fields, path, patch) {
  ...
  const rowCount = rows.length;
  if (rowCount < DIRTY_ARRAY_ROW_GROUP_MIN) return false;
  ...
  for (let rowOffset = 0; rowOffset < rowCount; rowOffset++) {
    ...
    for (let fieldIndex = 0, fieldCount = directKeys.length; fieldIndex < fieldCount; fieldIndex++) {
      ...
      if (shouldSetCollapsedChild(sourceValue, targetValue)) return false;
    ...
    for (let groupIndex = 0, groupCount = nestedKeys.length; groupIndex < groupCount; groupIndex++) {
      ...
      if (changed) {
        if (!isSmallDirtyNestedAssignValue(targetTopValue)) return false;
        if (assign === null) assign = {};
...
function prepareDirtyRowObjectFieldPlan(fields) {
  ...
  for (let fieldIndex = 0, fieldCount = fields.length; fieldIndex < fieldCount; fieldIndex++) {
    ...
    if (field.length === 0 || typeof topKey !== 'string') return null;

    if (field.length === 1) {
      if (directKeys.indexOf(topKey) === -1) directKeys[directKeys.length] = topKey;
      continue;
    }

    if (directKeys.indexOf(topKey) !== -1) continue;
...
function diffDirtyPaths(source, target, dirtyPaths, patch, keyCompare, getVersion, arrayKey) {
  ...
  for (let i = 0, length = dirtyPaths.length; i < length; i++) {
    ...
    if (expanded !== null) {
      ...
      for (let j = 0; j < i; j++) expanded[j] = dirtyPaths[j];
      expanded[i] = path;
    }
  }
...
function tryDiffDirtyCommonScalarArrayCellAssign(source, target, paths, patch) {
  ...
  if (count < ARRAY_TUPLE_ASSIGN_MIN) return false;
  ...
  if (last.length !== length) return false;
  for (let i = 0; i < prefixLength; i++) {
    if (first[i] !== last[i]) return false;
  }

  for (let i = 1; i < count - 1; i++) {
    if (paths[i].length !== length) return false;
  }
  ...
  const targetRows = readPathValue(target, path);
  if (
    count >= DENSE_DIRTY_CELL_PLAN_MIN &&
...
function tryDiffDirtyCommonScalarArrayIndexAssign(source, target, paths, patch) {
  ...
  if (pathCount < ARRAY_ASSIGN_MIN) return false;
  ...
  for (let i = 0; i < pathCount; i++) {
    ...
    if (dirtyPath.length !== length) return false;
    for (let j = 0; j < prefixLength; j++) {
      if (dirtyPath[j] !== first[j]) return false;
    }
    ...
    if (!isJsonScalarForReplaceRun(sourceValue) || !isJsonScalarForReplaceRun(targetValue)) return false;
    if (!sameJsonScalarOrRef(sourceValue, targetValue)) {
      if (indexes === null) {
        indexes = [];
        values = [];
      ...
      if (previousChangedIndex >= 0 && index !== previousChangedIndex + 1) contiguous = false;
      indexes[count] = index;
  ...
  if (count < ARRAY_ASSIGN_MIN) return false;
...
function tryDiffDirtyCommonArrayObjectAssign(source, target, paths, patch) {
  const count = paths.length;
  if (count < DIRTY_ARRAY_ROW_GROUP_MIN || count > DIRTY_PATH_GROUP_MAX) return false;

  const first = paths[0];
  const last = paths[count - 1];
  const rowDepth = commonPathPrefixLength(first, last);
  if (rowDepth < 0 || rowDepth >= first.length || rowDepth >= last.length) return false;
  if (count > 1 && paths[1][rowDepth] === first[rowDepth]) return false;

  for (let i = 1; i < count - 1; i++) {
    if (!pathHasPrefix(paths[i], first, rowDepth)) return false;
  }

  const path = first.slice(0, rowDepth);
  const sourceRows = readPathValue(source, path);
  const targetRows = readPathValue(target, path);
  if (!Array.isArray(sourceRows) || !Array.isArray(targetRows)) return false;

  return (
    tryDiffDirtyArrayObjectFieldAssign(sourceRows, targetRows, paths, 0, count, rowDepth, path, patch) ||
...
function pathHasPrefix(path, prefixPath, length) {
  if (path.length <= length) return false;
  for (let i = 0; i < length; i++) {
    if (path[i] !== prefixPath[i]) return false;
  }
  return true;
}
...
function hasPotentialDenseDirtyCellRows(paths, rowDepth) {
  ...
  for (let i = 0, length = paths.length; i < length; i++) {
    ...
    if (currentCount >= DENSE_DIRTY_CELL_ROW_MIN) return true;
  }
  return false;
}
...
function diffDirtyPathGroups(source, target, paths, start, end, depth, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  while (index < end) {
    ...
    if (depth + 1 === paths[index].length) {
      ...
      !tryDiffDirtyScalarArrayRowBandAssign(sourceValue, targetValue, paths, index, groupEnd, depth + 1, path, patch) &&
...
function tryDiffDirtyObjectLeafGroup(sourceObject, targetObject, paths, start, end, keyDepth, path, patch) {
  ...
  if (
    count < DIRTY_ARRAY_ROW_GROUP_MIN ||
    sourceObject === MISSING_PATH_VALUE ||
    targetObject === MISSING_PATH_VALUE ||
    sourceObject === null ||
    targetObject === null ||
...
function tryDiffDirtyScalarArrayRowBandAssign(sourceRows, targetRows, paths, start, end, rowDepth, path, patch) {
  if (!Array.isArray(sourceRows) || !Array.isArray(targetRows)) return false;
  const count = end - start;
  if (count < DIRTY_ARRAY_ROW_GROUP_MIN || sourceRows.length !== targetRows.length) return false;
  ...
  for (let i = start; i < end; i++) {
    const dirtyPath = paths[i];
    if (dirtyPath.length !== rowDepth + 1) return false;
    ...
    const sourceRow = sourceRows[rowIndex];
    const targetRow = targetRows[rowIndex];
    if (!isFullDirtyScalarRow(sourceRow, targetRow)) return false;

    if (rowWidth === 0) rowWidth = sourceRow.length;
    else if (sourceRow.length !== rowWidth) return false;
  ...
  if (indexes === null || indexes.length < DIRTY_ARRAY_ROW_GROUP_MIN) return false;
  patch[patch.length] = [OP_ARRAY_ASSIGN, path.slice(), indexes, values];
  return true;
}
...
function isFullDirtyScalarRow(sourceRow, targetRow) {
  ...
  if (length === 0 || length !== targetRow.length) return false;

  for (let i = 0; i < length; i++) {
    if (
      !hasOwn.call(sourceRow, i) ||
...
function tryDiffDirtyScalarArrayRowKeyframeAssign(sourceRows, targetRows, paths, start, end, rowDepth, path, patch) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (
      ...
      if (rowWidth === 0 || !isFixedScalarTupleRow(targetRow, rowWidth)) return false;
      if (width === 0) width = rowWidth;
      else if (rowWidth !== width) return false;

      const changed = countChangedScalarRowCells(sourceRow, targetRow, rowWidth);
      if (!isDenseDirtyRowSample(changed, rowWidth)) return false;
      denseSamples++;
    }
  ...
  if (indexes === null || denseSamples === 0) return false;
  patch[patch.length] = [OP_ARRAY_ASSIGN, path.slice(), indexes, values];
  return true;
}
...
function tryDiffDirtyScalarArrayCellAssign(sourceRows, targetRows, paths, start, end, rowDepth, path, patch) {
  ...
  if (pathCount < ARRAY_TUPLE_ASSIGN_MIN || sourceRows.length !== targetRows.length) return false;
  ...
  for (let i = start; i < end; i++) {
    const dirtyPath = paths[i];
    if (dirtyPath.length !== rowDepth + 2) return false;
...
function tryDiffDirtyScalarArrayCellPlan(sourceRows, targetRows, paths, start, end, rowDepth, path, patch) {
  ...
  const flushRow = () => {
    ...
    if (rowChangeCount === 0) return true;
    if (
      rowChangeCount >= DENSE_DIRTY_CELL_ROW_MIN &&
  ...
  for (let i = start; i < end; i++) {
    ...
    if (rowIndex !== currentRowIndex) {
      ...
      if (
        !Array.isArray(currentSourceRow) ||
...
function countChangedScalarRowCells(sourceRow, targetRow, width) {
  ...
  for (let i = 0; i < width; i++) {
    if (!sameJsonScalarOrRef(sourceRow[i], targetRow[i])) count++;
  }
  return count;
}
...
function emitDirtyScalarArrayRowPatch(sourceRow, targetRow, path, rowIndex, patch) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (sameJsonScalarOrRef(sourceValue, targetValue)) continue;
    if (!isJsonScalarForReplaceRun(sourceValue) || !isJsonScalarForReplaceRun(targetValue)) return false;
    ...
    if (i !== previous + 1) contiguous = count === 0;
    indexes[count] = i;
    values[count] = targetValue;
    count++;
    previous = i;
...
function collectChangedDirectScalarFields(sourceRow, targetRow, keys, values) {
  ...
  for (const key in sourceRow) {
    if (!hasOwn.call(sourceRow, key)) continue;
    if (!hasOwn.call(targetRow, key)) return -1;
    const sourceValue = sourceRow[key];
    const targetValue = targetRow[key];
    if (!sameJsonScalarOrRef(sourceValue, targetValue)) {
      if (!isJsonScalarForReplaceRun(sourceValue) || !isJsonScalarForReplaceRun(targetValue)) {
        if (boundedJsonEquals(sourceValue, targetValue)) continue;
        return -1;
      }
  ...
  for (const key in targetRow) {
    if (!hasOwn.call(targetRow, key)) continue;
    if (!hasOwn.call(sourceRow, key)) return -1;
  }
...
function tryDiffDirtyArrayObjectNestedAssign(sourceRows, targetRows, paths, start, end, rowDepth, path, patch) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (topKey === currentTopKey) continue;
...
function expandDirtyPath(source, target, path) {
  if (path.length < 4) return expandDirtyPathFromLeaf(source, target, path);
...
function diffArrays(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  if (lengthDelta > 1) {
    while (startIndex < sourceLength) {
      ...
      if (sourceValue !== targetValue || (sourceValue === 0 && 1 / sourceValue !== 1 / targetValue)) break;
      startIndex++;
    }
  ...
  if (
    shouldReplaceArray(source, target, commonLength, getVersion) &&
    (
      arrayKey === false ||
  ...
  if (
    ...
    if (
      hasSampledRecordRowScalarChange(source, target, commonLength) &&
...
function alignedArrayReaderKeyFullyMatches(source, target, commonLength, readKey) {
  for (let i = 0; i < commonLength; i++) {
    ...
    if (!isValidArrayKey(sourceKey) || sourceKey !== targetKey) return false;
  }
  return true;
}
...
function tryMakeTwoFieldInsert(values) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!isJsonScalarForReplaceRun(value0) || !isJsonScalarForReplaceRun(value1)) return null;
    values0[i] = value0;
    values1[i] = value1;
  }
...
function tryEmitScalarArrayReplace(patch, path, target) {
  ...
  for (let i = 0, length = target.length; i < length; i++) {
    ...
    if (!isJsonScalarForReplaceRun(value)) return false;
    values[i] = value;
  }
...
function tryKeyedArrayDiff(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  if (sourceKeys === null) return false;
  ...
  for (let i = 0; i < targetLength; i++) {
    ...
    if (sourceIndex !== undefined) {
      ...
      commonCount++;
      if (sourceIndex !== i) structuralChange = true;
    } else {
      structuralChange = true;
    }
  }

  if (!structuralChange || commonCount < KEYED_ARRAY_MIN) return false;
  if (commonCount * 2 < (sourceLength < targetLength ? sourceLength : targetLength)) return false;
...
function makeChangedObjectAssign(source, target) {
  if (!isRecordObject(source) || !isRecordObject(target)) return null;

  const sourceKeys = Object.keys(source);
  if (sourceKeys.length === 0 || sourceKeys.length > SMALL_OBJECT_KEY_LIMIT) return null;
  if (sourceKeys.length !== Object.keys(target).length) return null;

  let assign = null;
  let changed = 0;
  for (let i = 0, length = sourceKeys.length; i < length; i++) {
    const key = sourceKeys[i];
    if (!hasOwn.call(target, key)) return null;
    const sourceValue = source[key];
    const targetValue = target[key];
    if (sameJsonScalarOrRef(sourceValue, targetValue) || boundedJsonEquals(sourceValue, targetValue)) continue;
...
function inferArrayKey(source, target, policy?) {
  ...
  if (policyKey !== null) return policyKey;
  ...
  if (compositeKey !== null) return compositeKey;
...
function inferPolicyArrayKey(source, target, policy) {
  if (!isArrayKeyPolicy(policy) || policy.recordKeyCandidates === null) return null;
  const candidates = policy.recordKeyCandidates;
  for (let i = 0, length = candidates.length; i < length; i++) {
    const key = candidates[i];
    if (arrayKeyHasReorderSignal(source, target, key)) return makeObjectKeyReader(key);
  }
  return null;
}

function isScalarArrayKeyCandidate(value) {
  if (typeof value === 'string') return value.length > 0 && value.length <= STRUCTURAL_KEY_MAX_STRING;
...
function arrayKeyHasReorderSignal(source, target, key) {
  ...
  if (commonLength < KEYED_ARRAY_MIN) return false;
  ...
  for (let sample = 0; sample < sampleCount; sample++) {
    ...
    if (!isRecordObject(sourceValue) || !isRecordObject(targetValue)) return false;

    const sourceKey = sourceValue[key];
    const targetKey = targetValue[key];
    if (!isValidArrayKey(sourceKey) || !isValidArrayKey(targetKey)) return false;
    if (smallSeenHas(sourceSeen, sourceKey) || smallSeenHas(targetSeen, targetKey)) return false;
    sourceSeen[sourceSeen.length] = sourceKey;
    targetSeen[targetSeen.length] = targetKey;
    if (sourceKey !== targetKey) hasSignal = true;
  }
...
function compositeArrayKeyHasReorderSignal(source, target, key0, key1) {
  const commonLength = source.length < target.length ? source.length : target.length;
  if (commonLength < KEYED_ARRAY_MIN || commonLength > STRUCTURAL_ARRAY_KEY_MAX) return false;
  ...
  for (let sample = 0; sample < sampleCount; sample++) {
    ...
    if (sourceKey === undefined || targetKey === undefined) return false;
    if (smallSeenHas(sourceSeen, sourceKey) || smallSeenHas(targetSeen, targetKey)) return false;
    sourceSeen[sourceSeen.length] = sourceKey;
    targetSeen[targetSeen.length] = targetKey;
    if (sourceKey !== targetKey) hasSignal = true;
...
function inferObjectFieldArrayKey(source, target, requireReorderSignal) {
  const commonLength = source.length < target.length ? source.length : target.length;
  if (commonLength < KEYED_ARRAY_MIN || commonLength > OBJECT_FIELD_ARRAY_KEY_MAX) return null;

  const first = source[0];
  if (!isRecordObject(first)) return null;
  const keys = Object.keys(first);
  if (keys.length === 0 || keys.length > SMALL_OBJECT_KEY_LIMIT) return null;
...
function readObjectFieldArrayKey(value, key) {
  if (!isRecordObject(value) || !hasOwn.call(value, key)) return undefined;
...
function readCompositeObjectKey(value, key0, key1) {
  ...
  const value0 = value[key0];
  const value1 = value[key1];
  if (!isCompositeKeyPart(value0) || !isCompositeKeyPart(value1)) return undefined;
  return (
    'c' +
    key0.length + ':' + key0 +
...
function boundedJsonEquals(source, target) {
  if (source === target) return source !== 0 || 1 / source === 1 / target;
  const state = { nodes: 0 };
...
function boundedJsonEqualsInner(source, target, depth, state) {
  if (source === target) return source !== 0 || 1 / source === 1 / target;
  if (depth > STRUCTURAL_KEY_MAX_DEPTH || ++state.nodes > STRUCTURAL_KEY_MAX_NODES) return false;

  if (source === null || target === null) return false;

  const sourceType = typeof source;
  if (sourceType !== typeof target) return false;
  if (sourceType !== 'object') return false;

  const sourceIsArray = Array.isArray(source);
  if (sourceIsArray !== Array.isArray(target)) return false;

  if (sourceIsArray) {
    const length = source.length;
    if (length !== target.length || length > STRUCTURAL_KEY_MAX_ARRAY) return false;
    for (let i = 0; i < length; i++) {
      if (!hasOwn.call(source, i) || !hasOwn.call(target, i)) return false;
      if (!boundedJsonEqualsInner(source[i], target[i], depth + 1, state)) return false;
    }
    return true;
  }

  if (!isPlainStructuralObject(source) || !isPlainStructuralObject(target)) return false;

  let count = 0;
  for (const key in source) {
    if (!hasOwn.call(source, key)) continue;
    if (!hasOwn.call(target, key)) return false;
    if (++count > STRUCTURAL_KEY_MAX_KEYS) return false;
    if (!boundedJsonEqualsInner(source[key], target[key], depth + 1, state)) return false;
  }

  let targetCount = 0;
  for (const key in target) {
    if (hasOwn.call(target, key)) targetCount++;
  }
...
function inferStructuralArrayKey(source, target) {
  const commonLength = source.length < target.length ? source.length : target.length;
  if (commonLength < KEYED_ARRAY_MIN || commonLength > STRUCTURAL_ARRAY_KEY_MAX) return null;
  ...
  for (let sample = 0; sample < sampleCount; sample++) {
    ...
    const sourceValue = source[index];
    const targetValue = target[index];
    if (!isStructuralKeyCandidate(sourceValue) || !isStructuralKeyCandidate(targetValue)) return null;

    const sourceKey = readKey(sourceValue);
    const targetKey = readKey(targetValue);
    if (sourceKey === undefined || targetKey === undefined) return null;
    if (sourceKey !== targetKey) hasReorderSignal = true;
  }
...
function keySignalSampleCount(length) {
  if (length <= STRUCTURAL_KEY_SAMPLE_LIMIT) return length;
  const sqrtCount = Math.ceil(Math.sqrt(length));
  if (sqrtCount <= STRUCTURAL_KEY_SAMPLE_LIMIT) return STRUCTURAL_KEY_SAMPLE_LIMIT;
...
function makeStructuralArrayKeyReader() {
  ...
  const reader = (value) => {
    if (!isStructuralKeyCandidate(value)) return undefined;
    const cached = cache.get(value);
    if (cached !== undefined) return cached;
    const key = structuralArrayKey(value);
    if (key !== undefined) cache.set(value, key);
    return key;
  };
...
function tryPureSingleScalarMoveArrayDiff(source, target, path, patch) {
  const length = source.length;
  if (length < KEYED_ARRAY_MIN || length > PURE_KEYED_MOVE_MAX) return false;

  let start = 0;
  while (start < length) {
    const sourceValue = source[start];
    const targetValue = target[start];
    if (!isScalarArrayKeyCandidate(sourceValue) || !isScalarArrayKeyCandidate(targetValue)) return false;
    if (!sameJsonScalarOrRef(sourceValue, targetValue)) break;
    start++;
  }
  ...
  if (sameJsonScalarOrRef(source[end], target[start])) {
    for (let sourceIndex = start, targetIndex = start + 1; sourceIndex < end; sourceIndex++, targetIndex++) {
      if (!sameJsonScalarOrRef(source[sourceIndex], target[targetIndex])) return false;
    }
    patch[patch.length] = [OP_ARRAY_MOVE, path.slice(), end, start];
    return true;
  }
...
function tryPureSingleObjectKeyMoveArrayDiff(source, target, path, patch, key) {
  ...
  while (start < length) {
    ...
    if (!keyMatch) break;
    if (!boundedJsonEquals(source[start], target[start])) return false;
    start++;
  }
  ...
  while (end > start) {
    ...
    if (keyMatch === null) return false;
    if (!keyMatch) break;
    if (!boundedJsonEquals(source[end], target[end])) return false;
    end--;
  }

  const endToStartKeyMatch = sameObjectKey(source[end], target[start], key);
  if (endToStartKeyMatch === null) return false;
  if (endToStartKeyMatch && boundedJsonEquals(source[end], target[start])) {
    for (let sourceIndex = start, targetIndex = start + 1; sourceIndex < end; sourceIndex++, targetIndex++) {
      ...
      if (keyMatch !== true || !boundedJsonEquals(source[sourceIndex], target[targetIndex])) return false;
    }
    patch[patch.length] = [OP_ARRAY_MOVE, path.slice(), end, start];
    return true;
  }
  ...
  if (startToEndKeyMatch === null) return false;
...
function tryPureSingleCompositeMoveArrayDiff(source, target, path, patch, key0, key1) {
  const length = source.length;
  if (length < KEYED_ARRAY_MIN) return false;
  let start = 0;
...
function matchesLargeStructuralMoveEndToStart(source, target, start, end) {
  for (let sourceIndex = start, targetIndex = start + 1; sourceIndex < end; sourceIndex++, targetIndex++) {
    if (!sameStructuralMoveNode(source[sourceIndex], target[targetIndex])) return false;
  }
  return true;
}
...
function sameReaderKey(readKey, sourceValue, targetValue) {
  ...
  const sourceKey = readKey(sourceValue);
  const targetKey = readKey(targetValue);
  if (!isValidArrayKey(sourceKey) || !isValidArrayKey(targetKey)) return null;
  return sourceKey === targetKey;
}

function sameObjectKey(sourceValue, targetValue, key) {
  if (!isRecordObject(sourceValue) || !isRecordObject(targetValue)) return null;
  const sourceKey = sourceValue[key];
  const targetKey = targetValue[key];
  if (!isValidArrayKey(sourceKey) || !isValidArrayKey(targetKey)) return null;
  return sourceKey === targetKey;
...
function sameCompositeObjectKey(sourceValue, targetValue, key0, key1) {
  ...
  if (!isCompositeKeyPart(sourceValue0) || !isCompositeKeyPart(targetValue0)) return null;
  if (sourceValue0 !== targetValue0) return false;

  const sourceValue1 = sourceValue[key1];
  const targetValue1 = targetValue[key1];
  if (!isCompositeKeyPart(sourceValue1) || !isCompositeKeyPart(targetValue1)) return null;
...
function readUniqueArrayKeys(array, readKey) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!isValidArrayKey(key) || index.get(key) !== undefined) return null;
    keys[i] = key;
    index.set(key, i);
  }
...
function readUniqueObjectArrayKeys(array, key) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!isValidArrayKey(value) || index.get(value) !== undefined) return null;
    keys[i] = value;
    index.set(value, i);
  }
...
function tryLargePrimitiveShiftedArrayDiff(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  if (absDelta > LARGE_PRIMITIVE_ARRAY_SHIFT_LIMIT) return false;
  ...
  if (delta > 0) {
    ...
    if (
      shiftedLength < LARGE_PRIMITIVE_ARRAY_SHIFT_MIN_TAIL ||
...
function tryLargeShiftedArrayDiff(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  const delta = targetLength - sourceLength;
  const absDelta = delta < 0 ? -delta : delta;
  if (absDelta > LARGE_SHIFTED_ARRAY_LIMIT) return false;
  ...
  if (delta > 0) {
    const shiftedLength = sourceLength - index;
    if (
      shiftedLength < LARGE_SHIFTED_ARRAY_MIN_TAIL ||
...
function hasShiftedPrimitiveSignal(source, target, sourceStart, targetStart, length) {
  ...
  return (
    shiftedPrimitiveProbe(source, target, sourceStart, targetStart, 0) &&
...
function hasShiftedSignal(source, target, sourceStart, targetStart, length) {
  ...
  const twoThirds = (length * 2) / 3 | 0;
  return (
    sameShiftProbe(source[sourceStart], target[targetStart]) &&
...
function emitShiftedArrayDiffFromIndex(source, target, path, patch, keyCompare, getVersion, arrayKey, index) {
  ...
  if (delta > 0) {
    if (!sameShiftProbe(source[index], target[index + delta])) return false;

    const values = new Array(delta);
    for (let i = 0; i < delta; i++) values[i] = target[index + i];
  ...
  const deleteCount = -delta;
  if (!sameShiftProbe(source[index + deleteCount], target[index])) return false;
...
function tryTailArraySpliceDiff(source, target, path, patch) {
  ...
  if (sourceLength > 4 || start !== sourceLength - 1) return false;
  ...
  if (deleteCount + insertCount > SCALAR_REPLACE_RUN_LIMIT) return false;

  const values = new Array(insertCount);
  for (let i = 0; i < insertCount; i++) values[i] = target[start + i];
...
function tryBalancedRecordShiftDiff(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  if (deleteIndex - insertIndex < 8) return false;
...
function tryScalarReplaceRunDiff(source, target, path, patch) {
  ...
  if (count < 4 || count > SCALAR_REPLACE_RUN_LIMIT) return false;

  const values = new Array(count);
  for (let i = 0; i < count; i++) {
    const sourceValue = source[start + i];
    const targetValue = target[start + i];
    if (sameJsonScalarOrRef(sourceValue, targetValue)) return false;
    if (!isJsonScalarForReplaceRun(sourceValue) || !isJsonScalarForReplaceRun(targetValue)) {
      return false;
    }
...
function tryLargeScalarArrayRunSpliceDiff(source, target, path, patch) {
  const length = source.length;
  if (
    length < LARGE_SCALAR_RUN_ARRAY_MIN_LENGTH ||
  ...
  if (count === 0) return true;
  if (!shouldEmitScalarArrayRunSplices(count, runCount, path.length)) return false;
...
function shouldEmitScalarArrayRunSplices(count, runCount, pathDepth) {
  return (
    runCount >= ARRAY_RUN_SPLICE_MIN_RUNS &&
...
function tryTupleArrayAssignDiff(source, target, path, patch) {
  const length = source.length;
  if (length < ARRAY_TUPLE_ASSIGN_MIN_LENGTH) return false;

  const width = readTupleArrayWidth(source, target, length);
  if (width === 0) return false;

  if (tryFullTupleRowArrayAssignDiff(source, target, path, patch, width)) return true;
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (sameFixedScalarTupleRow(sourceRow, targetRow, width)) continue;
    ...
    if (rowChangeCount > 0) {
      if (i !== previousChangedRow + 1) contiguousChangedRows = changedRows === 0;
      changedRows++;
      if (rowChangeCount === width) fullChangedRows++;
...
function tryFullTupleRowArrayAssignDiff(source, target, path, patch, width) {
  ...
  for (let i = 0, length = source.length; i < length; i++) {
    const sourceRow = source[i];
    const targetRow = target[i];
    if (!isFixedScalarTupleRow(sourceRow, width) || !isFixedScalarTupleRow(targetRow, width)) return false;

    let changed = 0;
    for (let field = 0; field < width; field++) {
      if (!sameJsonScalarOrRef(sourceRow[field], targetRow[field])) changed++;
    }
  ...
  if (changedRows < 4 || changedRows > SCALAR_REPLACE_RUN_LIMIT) return false;
...
function tryRecordArrayFieldAssignDiff(source, target, path, patch) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const targetRow = target[i];
    if (sourceRow === targetRow) continue;
    if (!isPlainStructuralObject(sourceRow) || !isPlainStructuralObject(targetRow)) return false;

    const changedCount = collectChangedDirectScalarKeys(sourceRow, targetRow, changedKeys);
    if (changedCount === 0) continue;
    if (changedCount < 0 || changedCount > 16) return false;

    if (fieldSeen === null) fieldSeen = Object.create(null);
    for (let fieldIndex = 0; fieldIndex < changedCount; fieldIndex++) {
      ...
      if (fieldSeen[key] !== true) {
        ...
        if (fieldNames.length > 16) return false;
      }
    }
  ...
  for (let rowOffset = 0, rowCount = indexes.length; rowOffset < rowCount; rowOffset++) {
    ...
    for (let fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++) {
      ...
      if (!hasOwn.call(row, fieldNames[fieldIndex]) || !isJsonScalarForReplaceRun(value)) return false;
...
function tryRecordArrayNestedFieldAssignDiff(source, target, path, patch) {
  ...
  if (indexes.length < DIRTY_ARRAY_ROW_GROUP_MIN || fields.length === 0) return false;
  ...
  for (let rowOffset = 0, rowCount = indexes.length; rowOffset < rowCount; rowOffset++) {
    ...
    for (let fieldIndex = 0, fieldCount = fields.length; fieldIndex < fieldCount; fieldIndex++) {
      ...
      if (field.length === 1) {
        value = row[field[0]];
        if (!hasOwn.call(row, field[0])) return false;
      } else {
        const parent = row[field[0]];
        if (parent === null || typeof parent !== 'object' || !hasOwn.call(parent, field[1])) return false;
...
function tryRecordArraySingleNestedFieldAssignDiff(source, target, path, patch, field, minChangedRows) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const targetRow = target[i];
    if (sourceRow === targetRow) continue;
    if (plan.fastKind !== 1 && (!isPlainStructuralObject(sourceRow) || !isPlainStructuralObject(targetRow))) return false;

    const rowState = compareRecordRowWithSingleNestedFieldPlan(sourceRow, targetRow, plan);
    if (rowState < 0) return false;
    if (rowState > 0) {
  ...
  if (indexes.length === 0) return true;
  if (indexes.length < minChangedRows) return false;
...
function tryRecordArraySingleNestedFieldAssignDiffFast1(source, target, path, patch, field, plan, minChangedRows) {
  ...
  for (let i = 0, length = source.length; i < length; i++) {
    ...
    if (
      !hasExactOwnKeys3(sourceRow, topScalarKey, arrayKey, topKey) ||
    ...
    if (
      !hasExactOwnKeys3(sourceParent, childKey, parentScalarKey, groupKey) ||
    ...
    if (
      !hasExactOwnKeys2(sourceObject, groupKey0, groupKey1) ||
...
function readSingleNestedScalarFieldRowPlan(row, field) {
  if (!isPlainStructuralObject(row)) return null;
  ...
  for (const key in row) {
    ...
    if (key === topKey) {
      if (!isPlainStructuralObject(value)) return null;
      sawTop = true;
      for (const child in value) {
        if (!hasOwn.call(value, child)) continue;
        parentCount++;
        const childValue = value[child];
        if (child === childKey) {
          if (!isJsonScalarForReplaceRun(childValue)) return null;
          sawChild = true;
  ...
  if (!sawTop || !sawChild) return null;
...
function readScalarObjectGroup(key, object) {
  ...
  for (const childKey in object) {
    if (!hasOwn.call(object, childKey)) continue;
    const value = object[childKey];
    if (!isJsonScalarForReplaceRun(value)) return null;
...
function readScalarArrayFieldPlan(key, array) {
  ...
  for (let i = 0; i < length; i++) {
    if (!hasOwn.call(array, i) || !isJsonScalarForReplaceRun(array[i])) return null;
  }
  return { key, length };
}
...
function compareRecordRowWithSingleNestedFieldPlan(sourceRow, targetRow, plan) {
  ...
  if (
    countOwnKeysUntil(sourceRow, plan.topCount + 1) !== plan.topCount ||
  ...
  if (
    !isPlainStructuralObject(sourceParent) ||
  ...
  for (let i = 0, length = parentObjectGroups.length; i < length; i++) {
    if (!sameScalarObjectGroup(sourceParent, targetParent, parentObjectGroups[i])) return -1;
  }

  const sourceValue = sourceParent[plan.childKey];
  const targetValue = targetParent[plan.childKey];
  if (!hasOwn.call(sourceParent, plan.childKey) || !hasOwn.call(targetParent, plan.childKey)) return -1;
  if (sameJsonScalarOrRef(sourceValue, targetValue)) return 0;
...
function compareRecordRowWithSingleNestedFieldPlanFast1(sourceRow, targetRow, plan) {
  ...
  const arrayKey = plan.topArrayKeys[0].key;
  if (
    !hasExactOwnKeys3(sourceRow, topScalarKey, arrayKey, plan.topKey) ||
  ...
  const group = plan.parentObjectGroups[0];
  if (
    !hasExactOwnKeys3(sourceParent, plan.childKey, parentScalarKey, group.key) ||
...
function hasExactOwnKeys3(object, key0, key1, key2) {
  ...
  for (const key in object) {
    ...
    if (key !== key0 && key !== key1 && key !== key2) return false;
    count++;
  }
  return count === 3;
}
...
function hasExactOwnKeys2(object, key0, key1) {
  ...
  for (const key in object) {
    if (!hasOwn.call(object, key)) continue;
    if (key !== key0 && key !== key1) return false;
    count++;
  }
  return count === 2;
}
...
function sameScalarObjectGroup(sourceParent, targetParent, group) {
  ...
  const targetObject = targetParent[group.key];
  if (
    !isPlainStructuralObject(sourceObject) ||
...
function collectChangedDirectOrNestedScalarFields(sourceRow, targetRow, fields) {
  ...
  for (const key in sourceRow) {
    ...
    if (isPlainStructuralObject(sourceValue) && isPlainStructuralObject(targetValue)) {
      ...
      if (nestedCount < 0) return -1;
      continue;
    }

    if (!boundedJsonEquals(sourceValue, targetValue)) return -1;
  }

  for (const key in targetRow) {
    if (hasOwn.call(targetRow, key) && !hasOwn.call(sourceRow, key)) return -1;
  }
...
function collectChangedNestedScalarFields(topKey, sourceObject, targetObject, fields) {
  ...
  for (const key in sourceObject) {
    if (!hasOwn.call(sourceObject, key)) continue;
    if (!hasOwn.call(targetObject, key)) return -1;
    const sourceValue = sourceObject[key];
    const targetValue = targetObject[key];
    if (sameJsonScalarOrRef(sourceValue, targetValue)) continue;
  ...
  for (const key in targetObject) {
    if (hasOwn.call(targetObject, key) && !hasOwn.call(sourceObject, key)) return -1;
  }
...
function hasSampledRecordRowScalarChange(source, target, length) {
  ...
  for (let sample = 0; sample < 9; sample++) {
    ...
    if (sourceRow === targetRow) continue;
    if (!isPlainStructuralObject(sourceRow) || !isPlainStructuralObject(targetRow)) continue;
    for (const key in sourceRow) {
      if (!hasOwn.call(sourceRow, key) || !hasOwn.call(targetRow, key)) continue;
      const sourceValue = sourceRow[key];
      const targetValue = targetRow[key];
      if (
        !sameJsonScalarOrRef(sourceValue, targetValue) &&
...
function readSampledRecordRowNestedScalarField(source, target, length) {
  ...
  for (let sample = 0; sample < sampleCount; sample++) {
    ...
    for (const key in sourceRow) {
      ...
      const sourceValue = sourceRow[key];
      const targetValue = targetRow[key];
      if (!isPlainStructuralObject(sourceValue) || !isPlainStructuralObject(targetValue)) continue;
      for (const childKey in sourceValue) {
        if (!hasOwn.call(sourceValue, childKey) || !hasOwn.call(targetValue, childKey)) continue;
        const sourceChild = sourceValue[childKey];
        const targetChild = targetValue[childKey];
        if (
          !sameJsonScalarOrRef(sourceChild, targetChild) &&
...
function readTupleArrayWidth(source, target, length) {
  ...
  for (let sample = 0; sample < sampleCount; sample++) {
    ...
    else if (sourceWidth !== width) return 0;
    if (!isFixedScalarTupleRow(target[index], width)) return 0;
  }
  return width;
}
...
function readScalarTupleRowWidth(row) {
  ...
  if (length < 2 || length > ARRAY_TUPLE_ASSIGN_MAX_WIDTH) return 0;
  for (let i = 0; i < length; i++) {
    if (!hasOwn.call(row, i) || !isJsonScalarForReplaceRun(row[i])) return 0;
  }
  return length;
}
...
function readKeyframeScalarRowWidth(row) {
  if (!Array.isArray(row)) return 0;
  const length = row.length;
  if (length < 2 || length > DIRTY_ARRAY_KEYFRAME_ROW_MAX_WIDTH) return 0;
  for (let i = 0; i < length; i++) {
    if (!hasOwn.call(row, i) || !isJsonScalarForReplaceRun(row[i])) return 0;
  }
  return length;
...
function isFixedScalarTupleRow(row, width) {
  ...
  for (let i = 0; i < width; i++) {
    if (!hasOwn.call(row, i) || !isJsonScalarForReplaceRun(row[i])) return false;
  }
  return true;
}
...
function sameFixedScalarTupleRow(sourceRow, targetRow, width) {
  ...
  for (let i = 0; i < width; i++) {
    ...
    if (!isJsonScalarForReplaceRun(value) || !sameJsonScalarOrRef(value, targetRow[i])) return false;
  }
  return true;
}
...
function tryTwoKeyRecordArrayDiff(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  for (let i = 0, length = source.length; i < length; i++) {
    ...
    if (sourceValue === targetValue) {
      ...
      const targetValue0 = targetValue[key0];
      if (
        sameJsonScalarOrRef(sourceValue0, targetValue0) &&
...
function emitArrayObjectAssignFromTarget(patch, path, indexes, assigns, targetRows) {
  if (tryEmitArrayObjectAssignAsFieldAssign(patch, path, indexes, assigns, targetRows)) return;
...
function tryEmitArrayObjectAssignAsFieldAssign(patch, path, indexes, assigns, targetRows) {
  ...
  for (let rowOffset = 0, rowCount = assigns.length; rowOffset < rowCount; rowOffset++) {
    ...
    for (let i = 0, length = keys.length; i < length; i++) {
      const key = keys[i];
      if (!isJsonScalarForReplaceRun(assign[key])) return false;
      if (fieldSeen[key] !== true) {
        fieldSeen[key] = true;
        fieldNames[fieldNames.length] = key;
        if (fieldNames.length > 16) return false;
  ...
  for (let rowOffset = 0, rowCount = indexes.length; rowOffset < rowCount; rowOffset++) {
    ...
    for (let fieldIndex = 0, fieldCount = fieldNames.length; fieldIndex < fieldCount; fieldIndex++) {
      ...
      const value = row[key];
      if (!hasOwn.call(row, key) || !isJsonScalarForReplaceRun(value)) return false;
      values[values.length] = value;
    }
  }
...
function makeFlatObjectAssign(source, target) {
  ...
  for (const key in source) {
    ...
    const targetValue = target[key];
    if (sameJsonScalarOrRef(sourceValue, targetValue)) continue;
    if (!isJsonScalarForReplaceRun(targetValue)) return null;
    if (assign === null) assign = {};
    setOwnValue(assign, key, targetValue);
    count++;
  }
  ...
  for (const key in target) {
    if (!hasOwn.call(target, key) || hasOwn.call(source, key)) continue;
    if (!isJsonScalarForReplaceRun(target[key])) return null;
    if (assign === null) assign = {};
...
function sameShiftProbe(source, target) {
  if (sameJsonScalarOrRef(source, target)) return true;
  if (
    source !== null &&
    target !== null &&
...
function shouldReplaceArray(source, target, commonLength, getVersion) {
  if (commonLength < 64) return false;
  if (
    getVersion !== null &&
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const targetValue = target[index];
    if (
      sameShiftProbe(sourceValue, targetValue) ||
      boundedJsonEquals(sourceValue, targetValue)
    ) {
      matches++;
    }
...
function shouldUseSparseArrayDiff(source, target, commonLength) {
  if (commonLength < 65536) return false;
  if (commonLength === 0) return false;

  const last = commonLength - 1;
  for (let i = 0; i < 8; i++) {
    const index = Math.floor(last * i / 7);
    if (!hasOwn.call(source, index) || !hasOwn.call(target, index)) return true;
  }
...
function diffSparseArrays(source, target, path, patch, keyCompare, getVersion, arrayKey) {
  ...
  for (let i = 0, length = sourceKeys.length; i < length; i++) {
    ...
    if (index >= targetLength) continue;
...
function sameEqualObjectSubtree(source, target) {
  ...
  if (sourceCount < EQUAL_OBJECT_SUBTREE_MIN_KEYS || sourceCount > SMALL_OBJECT_KEY_LIMIT) return false;
...
function shouldSetCollapsedChild(source, target) {
  ...
  if (sourceCount >= 32) {
    ...
    if (targetCount <= 8 && sourceCount > targetCount * 4) return true;
  }
...
function sameKeys(sourceKeys, targetKeys) {
  for (let i = 1, length = sourceKeys.length - 1; i < length; i++) {
    if (sourceKeys[i] !== targetKeys[i]) return false;
  }
  return true;
}
...
function emitStringSplice(patch, path, source, target) {
  ...
  if (
    (start > 0 || sourceEnd < sourceLength || targetEnd < targetLength) &&
...
function tryEmitEqualLengthStringSubstitutions(patch, path, source, target, start, end) {
  ...
  while (index < end) {
    while (index < end && source.charCodeAt(index) === target.charCodeAt(index)) index++;
    if (index >= end) break;
...
function emitStringInsert(patch, path, source, start, insert) {
  ...
  if (tryEmitStringCopyPrefix(patch, path, source, start, 0, insert, 1)) return;
  if (tryEmitStringCopySuffix(patch, path, source, start, 0, insert, 1)) return;
...
function tryEmitStringCopySuffix(patch, path, source, start, deleteCount, insert, minPrefix) {
  if (insert.length < STRING_COPY_MIN + minPrefix) return false;
...
function tryEmitStringRotationSplice(patch, path, source, target, start, end) {
  ...
  if (length < STRING_ROTATION_MIN || length > STRING_ROTATION_MAX) return false;

  const firstCandidate = source.indexOf(target.charAt(start), start + 1);
  if (firstCandidate < 0 || firstCandidate >= end) return false;
  ...
  if (shift <= 0 || shift >= length) return false;
...
function flatPrimitiveObjectEquals(source, target) {
  ...
  for (let i = 0; i < length; i++) {
    const key = sourceKeys[i];
    if (key !== targetKeys[i]) return false;
...
function flatPrimitiveArrayEquals(source, target) {
  ...
  const length = source.length;
  if (length !== target.length || length > FLAT_ARRAY_SHIFT_PROBE_MAX) return false;
...
function flatTwoKeyObjectEquals(source, target, key0, key1) {
  ...
  return (
    hasExactlyTwoOwnKeys(source, key0, key1) &&
...
function readDirtyPaths(options) {
  if (!options || options.dirtyPaths === undefined) return undefined;
...
function readDirtyRows(options) {
  if (!options || options.dirtyRows === undefined) return undefined;
  if (options.dirtyRows === null) return null;
...
function normalizeDirtyPaths(paths, validate) {
  ...
  if (paths.length === 1) {
    const path = paths[0];
    if (validate) validateDirtyPath(path);
...
function readKeyCompare(options) {
  ...
  if (typeof options.keyCompare === 'function') return options.keyCompare;
  if (typeof options.stable === 'function') return options.stable;
...
function readArrayKey(options) {
  if (!options) return null;
  if (options.arrayKey === false || options.autoArrayKey === false) return false;
  ...
  if (options.arrayKey !== undefined && options.arrayKey !== null && options.arrayKey !== true) {
    if (typeof options.arrayKey === 'function') return options.arrayKey;
