...
export function applyJsonPatch(value: JsonValue, patch: JsonPatch, options?: ApplyOptions): JsonValue | undefined {
  if (!Array.isArray(patch)) throw new TypeError('JSON Patch document must be an array');
  ...
  if (patch.length >= JSON_PATCH_ROW_FIELD_REPLACE_MIN) {
    ...
    if (fast !== JSON_PATCH_FAST_UNSUPPORTED) return fast;
  }
...
function tryApplyObjectFieldPatch(root, patch, cloneValues) {
  if (root === null || typeof root !== 'object') return JSON_PATCH_FAST_UNSUPPORTED;
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    const op = patch[i];
    if ((op.op !== 'add' && op.op !== 'replace') || typeof op.path !== 'string') return JSON_PATCH_FAST_UNSUPPORTED;

    const path = getCachedPointerPath(op.path);
    if (path.length === 0) return JSON_PATCH_FAST_UNSUPPORTED;

    if (i === 0) {
      firstPath = path;
      parentPathLength = path.length - 1;
      if (parentPathLength < 2) return JSON_PATCH_FAST_UNSUPPORTED;
      parent = readFastJsonPatchPathValue(root, path, parentPathLength);
      if (parent === null || typeof parent !== 'object' || Array.isArray(parent)) return JSON_PATCH_FAST_UNSUPPORTED;
    } else if (
    ...
    if (op.op === 'replace' && !hasOwn(parent, key)) return JSON_PATCH_FAST_UNSUPPORTED;
    keys[i] = key;
    values[i] = op.value;
  }
...
function tryApplyRowFieldReplacePatch(root, patch, cloneValues) {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    const op = patch[i];
    if (op.op !== 'replace' || typeof op.path !== 'string') return JSON_PATCH_FAST_UNSUPPORTED;

    const path = getCachedPointerPath(op.path);
    if (path.length < 3) return JSON_PATCH_FAST_UNSUPPORTED;
    const rowIndexOffset = path.length - 2;
    if (typeof path[rowIndexOffset] !== 'number') return JSON_PATCH_FAST_UNSUPPORTED;

    if (i === 0) {
      firstPath = path;
      collectionPathLength = rowIndexOffset;
      rows = readFastJsonPatchPathValue(root, path, collectionPathLength);
      if (!Array.isArray(rows)) return JSON_PATCH_FAST_UNSUPPORTED;
    } else if (
      path.length !== firstPath.length ||
    ...
    if (row === null || row === undefined || typeof row !== 'object') return JSON_PATCH_FAST_UNSUPPORTED;
    const fieldKey = path[path.length - 1];
    const rowIsArray = Array.isArray(row);

    if (rowIsArray) {
      if (typeof fieldKey !== 'number' || fieldKey < 0 || fieldKey >= row.length) return JSON_PATCH_FAST_UNSUPPORTED;
    ...
    keys[i] = fieldKey;
    if (rowIsArray) {
      if (targetIsArray === null) targetIsArray = new Array(patch.length);
...
function readFastJsonPatchPathValue(root, path, length) {
  ...
  for (let i = 0; i < length; i++) {
    if (node === null || node === undefined || typeof node !== 'object') return JSON_PATCH_FAST_UNSUPPORTED;
    const key = path[i];
    if (Array.isArray(node)) {
      if (typeof key !== 'number' || key < 0 || key >= node.length) return JSON_PATCH_FAST_UNSUPPORTED;
      node = node[key];
    } else {
      if (!hasOwn(node, key)) return JSON_PATCH_FAST_UNSUPPORTED;
      node = node[key];
    }
  }
  return node;
...
function cloneJsonPatchContainerPath(root, path, depth, wideLeafObject = false) {
  ...
  if (clonedRoot === null) return null;
  if (depth === 0) return [clonedRoot, clonedRoot];

  let originalNode = root;
  let clonedNode = clonedRoot;
  for (let i = 0; i < depth; i++) {
    if (originalNode === null || originalNode === undefined || typeof originalNode !== 'object') return null;
    const key = path[i];
    let child;
    if (Array.isArray(originalNode)) {
      if (typeof key !== 'number' || key < 0 || key >= originalNode.length) return null;
      ...
      if (!hasOwn(originalNode, key)) return null;
      child = originalNode[key];
...
export function applyJsonPatchImmutable(value: JsonValue, patch: JsonPatch): JsonValue | undefined {
  if (!Array.isArray(patch)) throw new TypeError('JSON Patch document must be an array');
...
function tryApplyImmutableObjectFieldPatch(root, patch) {
  ...
  if (cloned === null) return JSON_PATCH_FAST_UNSUPPORTED;
...
function tryApplyImmutableRowFieldReplacePatch(root, patch) {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (rowIndex <= lastRowIndex) rowsAreStrictlyIncreasing = false;
  ...
  if (cloned === null || !Array.isArray(cloned[1])) return JSON_PATCH_FAST_UNSUPPORTED;
  ...
  if (rowsAreStrictlyIncreasing) {
    for (let i = 0, length = rowIndexes.length; i < length; i++) {
      ...
      if (row === null) return JSON_PATCH_FAST_UNSUPPORTED;
...
function replacePointerValue(root, pointer, value) {
  ...
  if (Array.isArray(parent)) {
    ...
    if (!hasOwn(parent, key)) throw new TypeError('JSON Patch path does not exist: ' + pointer);
    setOwnValue(parent, key, value);
  }
  return root;
}
...
function getParentAtPath(root, path, pointer) {
  ...
  for (let i = 0, length = path.length - 1; i < length; i++) {
    ...
    if (Array.isArray(node)) {
      ...
      const key = path[i];
      if (!hasOwn(node, key)) throw new TypeError('JSON Patch parent path does not exist: ' + pointer);
      node = node[key];
    }
  }
...
function getRequiredPath(root, path, pointer) {
  ...
  const parent = getParentAtPath(root, path, pointer);
  const key = path[path.length - 1];
  if (Array.isArray(parent)) return parent[readArrayIndex(key, parent.length, false)];
  if (!hasOwn(parent, key)) throw new TypeError('JSON Patch path does not exist: ' + pointer);
  return parent[key];
}
...
function assertMoveTarget(from, path) {
  ...
  if (fromPath.length >= targetPath.length) return;

  for (let i = 0, length = fromPath.length; i < length; i++) {
    if (fromPath[i] !== targetPath[i]) return;
  }
