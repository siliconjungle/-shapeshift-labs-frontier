...
import type { NormalizeOptions, Patch } from './types.js';
...
export function normalizePatch(patch: Patch, options?: NormalizeOptions): Patch {
  ...
  for (let i = 0, length = patch.length; i < length; i++) {
    ...
    if (assignmentRun !== null) {
      ...
      if (!isNoOp(op)) appendCopiedNormalized(out, op);
...
function extractAssignmentRun(patch, start) {
  ...
  if (code === OP_ARRAY_ASSIGN) return extractArrayAssignRun(patch, start);
  if (code === OP_ARRAY_OBJECT_ASSIGN) return extractArrayObjectAssignRun(patch, start);
  if (code === OP_ARRAY_OBJECT_FIELD_ASSIGN) return extractArrayObjectFieldAssignRun(patch, start);
  return null;
}
...
function extractSiblingSetRun(patch, start) {
  ...
  if (first[0] !== OP_SET) return null;

  const firstPath = first[1];
  const depth = firstPath.length - 1;
  if (depth < 0 || typeof firstPath[depth] !== 'string') return null;

  let end = start + 1;
  let values = null;
  while (end < patch.length) {
    const next = patch[end];
    if (next[0] !== OP_SET) break;

    const nextPath = next[1];
    if (
      nextPath.length !== firstPath.length ||
...
function isNoOp(op) {
  const code = op[0];
  if (code === OP_ASSIGN) return Object.keys(op[2]).length === 0;
  if (code === OP_APPEND) return op[2].length === 0;
  if (code === OP_ARRAY_SPLICE) return op[3] === 0 && op[4].length === 0;
  if (code === OP_STRING_SPLICE) return op[3] === 0 && op[4] === '';
  if (code === OP_ARRAY_MOVE) return op[2] === op[3];
  if (code === OP_STRING_COPY) return op[4] === 0;
  if (code === OP_ARRAY_ASSIGN) return op[2].length === 0;
  if (code === OP_ARRAY_OBJECT_ASSIGN) return op[2].length === 0;
  if (code === OP_ARRAY_TUPLE_ASSIGN) return op[2].length === 0;
  if (code === OP_ARRAY_OBJECT_FIELD_ASSIGN) return op[2].length === 0 || op[3].length === 0;
  if (code === OP_SCALAR_ARRAY_REPLACE) return false;
  if (code === OP_ARRAY_TWO_FIELD_INSERT) return op[5].length === 0;
  return false;
}
...
function copyOperation(op) {
  ...
  if (code === OP_SET) return [code, path, op[2]];
  if (code === OP_REMOVE) return [code, path];
  if (code === OP_TRUNCATE) return [code, path, op[2]];
  if (code === OP_APPEND) return [code, path, op[2].slice()];
  if (code === OP_ASSIGN) return [code, path, copyAssignValues(op[2])];
  if (code === OP_STRING_SPLICE) return [code, path, op[2], op[3], op[4]];
  if (code === OP_ARRAY_SPLICE) return [code, path, op[2], op[3], op[4].slice()];
  if (code === OP_ARRAY_MOVE) return [code, path, op[2], op[3]];
  if (code === OP_STRING_COPY) return [code, path, op[2], op[3], op[4]];
  if (code === OP_ARRAY_ASSIGN) return [code, path, op[2].slice(), op[3].slice()];
  if (code === OP_ARRAY_OBJECT_ASSIGN) return [code, path, op[2].slice(), copyAssignList(op[3])];
  if (code === OP_ARRAY_TUPLE_ASSIGN) return [code, path, op[2].slice(), op[3].slice(), op[4].slice()];
  if (code === OP_ARRAY_OBJECT_FIELD_ASSIGN) return [code, path, op[2].slice(), copyFieldPaths(op[3]), op[4].slice()];
  if (code === OP_SCALAR_ARRAY_REPLACE) return [code, path, op[2].slice()];
...
function mergeArraySplice(out, last, next) {
  ...
  if (
    ...
    if (lastValues.length === 0) out.pop();
    return true;
  }
...
function mergeStringCopy(last, next) {
  ...
  if (
    nextTargetStart === lastTargetStart + lastLength &&
...
function extractArrayAssignRun(patch, start) {
  ...
  if (firstIndexes.length !== firstValues.length) return null;
  ...
  while (end < patch.length) {
    const next = patch[end];
    if (next[0] !== OP_ARRAY_ASSIGN || !samePath(path, next[1])) break;
    const nextIndexes = next[2];
    const nextValues = next[3];
    if (nextIndexes.length !== nextValues.length) break;

    if (
      (nextIndexes.length > 1 && !isStrictlyIncreasingIndexes(nextIndexes)) ||
    ...
    if (nextIndexes.length !== 0) lastIndex = nextIndexes[nextIndexes.length - 1];
    end++;
  }

  if (needsCompaction) {
    const compacted = makeLastWriteArrayAssign(path, indexes, values);
    if (compacted !== null) return { end, op: compacted };
  }

  if (end > start + 1) return { end, op: [OP_ARRAY_ASSIGN, path, indexes, values] };
  return null;
}
...
function extractArrayObjectAssignRun(patch, start) {
  ...
  if (!isStrictlyIncreasingIndexes(firstIndexes) || firstIndexes.length !== firstAssigns.length) return null;
  ...
  while (end < patch.length) {
    const next = patch[end];
    if (next[0] !== OP_ARRAY_OBJECT_ASSIGN || !samePath(path, next[1])) break;
    const nextIndexes = next[2];
    const nextAssigns = next[3];
    if (
      nextIndexes.length !== nextAssigns.length ||
  ...
  if (indexes.length >= ARRAY_OBJECT_ASSIGN_FIELD_MIN_ROWS) {
    ...
    if (fieldOp !== null) return { end, op: fieldOp };
  }

  if (end > start + 1) return { end, op: [OP_ARRAY_OBJECT_ASSIGN, path, indexes, assigns] };
  return null;
}
...
function extractArrayObjectFieldAssignRun(patch, start) {
  ...
  const firstIndexes = first[2];
  if (first[4].length !== firstIndexes.length * fields.length) return null;
  ...
  while (end < patch.length) {
    const next = patch[end];
    if (
      next[0] !== OP_ARRAY_OBJECT_FIELD_ASSIGN ||
    ...
    const nextIndexes = next[2];
    if (
      next[4].length !== nextIndexes.length * fields.length
    ) {
      break;
    }
  ...
  if (end > start + 1) return { end, op: [OP_ARRAY_OBJECT_FIELD_ASSIGN, path, indexes, fields, values] };
  return null;
}
...
function makeLastWriteArrayObjectFieldAssign(path, indexes, fields, values) {
  if (indexes.length === 0 || fields.length === 0) return null;
...
function makeArrayObjectFieldAssign(path, indexes, assigns) {
  ...
  if (!isPlainRecord(firstAssign)) return null;

  const keys = Object.keys(firstAssign);
  const fieldCount = keys.length;
  if (fieldCount === 0 || fieldCount > ARRAY_OBJECT_ASSIGN_FIELD_MAX_FIELDS) return null;

  const values = [];
  for (let row = 0, rowCount = assigns.length; row < rowCount; row++) {
    const assign = assigns[row];
    if (!isPlainRecord(assign) || !hasExactOwnKeys(assign, keys, fieldCount)) return null;
    for (let field = 0; field < fieldCount; field++) {
  ...
  for (let field = 0; field < fieldCount; field++) fields[field] = [keys[field]];
